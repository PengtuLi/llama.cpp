...
// GTODO: this is a demo version generated by AI

// 内层切块：对一块 [tokens x neurons] 范围，按 mask 决定哪些 dot-product 要执行
static void ggml_compute_forward_mul_mat_sparse_one_chunk(
    const struct ggml_compute_params * params,
          struct ggml_tensor * dst,        // dst->shape = [token_batch, n_ff_dim]
    const enum ggml_type      type,      // src0/src1 的数据类型
    const int64_t             ir0_start, // token 起始 idx
    const int64_t             ir0_end,   // token 结束 idx（不含）
    const int64_t             ir1_start, // neuron 起始 idx
    const int64_t             ir1_end)   // neuron 结束 idx（不含）
{

    const struct ggml_tensor * src0 = dst->src[0]; // [token_batch, hidden_dim]
    const struct ggml_tensor * src1 = dst->src[1]; // [hidden_dim, n_ff_dim]
    const struct ggml_tensor * idx  = dst->src[2]; // [n_ff_dim, token_batch]
    const struct ggml_tensor * mask = dst->src[3]; // [n_ff_dim]
    ...
    const size_t row_size   = ggml_row_size(vec_dot_type, ne00); // hidden_dim

    const int64_t blck_0 = 16;  // tokens 方向
    const int64_t blck_1 = 16;  // neurons 方向

    // 临时缓存一个小 tile 的 dot 结果
    float tmp[16];

    for (int64_t i1 = ir1_start; i1 < ir1_end; i1 += blck_1) {
        for (int64_t i0 = ir0_start; i0 < ir0_end; i0 += blck_0) {
            // 对一个小 tile 内的每个 neuron
            for (int64_t neuron = i1; neuron < i1 + blck_1 && neuron < ir1_end; neuron++) {
                // src1 的这一列指针
                const char * src1_col = (const char *)wdata + neuron * row_size;

                // 对这一 neuron，在 token 方向做 dot
                int cnt = 0;
                for (int64_t tok = i0; tok < i0 + blck_0 && tok < ir0_end; tok++) {
                    // mask 判断
                    const float idx_i = *(const float *)
                        ((const char *)idx->data
                         + neuron * idx->nb[0]
                         + tok    * idx->nb[1]);

                    const bool mask_i = *(const int64_t *) // GTODO: type of mask??
                        ((const char *)mask->data
                         + neuron * mask->nb[0]
                         + tok    * mask->nb[1]);

                    if (idx_i < 0.5f || mask_i == 0) {   // sparse or on GPU -> skip
                        tmp[cnt++] = 0.0f;
                    } else {
                        const char * src0_row = (const char *)src0->data
                            + tok * nb1; // hidden_dim stride
                        float acc;
                        vec_dot(ne00, &acc,
                                /*off0=*/0, src0_row, /*st0=*/0,
                                src1_col,/*st1=*/0,
                                /*n=*/0);
                        tmp[cnt++] = acc;
                    }
...
// -----------------------------------------------------------------------------
// main function of sparse mulmat
static void ggml_compute_forward_mul_mat_sparse(
    const struct ggml_compute_params * params,
          struct ggml_tensor * dst) // src0, src1, src2(mask)
{
    const struct ggml_tensor * src0 = dst->src[0];
    const struct ggml_tensor * src1 = dst->src[1];
    const struct ggml_tensor * idx  = dst->src[2]; // sparse_idx
    const struct ggml_tensor * mask = dst->src[3]; // gpu_neu_mask
    ...
    // define dimensioin of the result
    const int64_t nr0 = idx->ne[1]; // token_batch
    const int64_t nr1 = idx->ne[0]; // n_ff_dim
    ...
    // 每个线程从 chunk = ith 开始，之后 atomic 争抢
    int64_t current_chunk = ith;
    while (current_chunk < nchunk0 * nchunk1) {
        const int64_t i0 = current_chunk % nchunk0;
        const int64_t i1 = current_chunk / nchunk0;
        ...
        ggml_compute_forward_mul_mat_sparse_one_chunk(
            params, dst,
            src0->type,
            ir0_start, ir0_end,
            ir1_start, ir1_end
        ...
        current_chunk = atomic_fetch_add_explicit(
            &params->threadpool->current_chunk, 1,
            memory_order_relaxed);
    }
}
